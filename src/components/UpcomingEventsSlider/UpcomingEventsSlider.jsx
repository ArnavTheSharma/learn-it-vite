...
const UpcomingEventsSlider = ({ events }) => {
    ...
    return (
    <div className="w-full relative py-6 flex items-center overflow-hidden">
    {/* Left arrow */}
    <button
        onClick={prev}
        aria-label="Previous"
    ...
    >
        <svg className="w-7 h-7 text-[#DD6C73]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
    </button>

    {/* Carousel */}
    <div className="flex gap-6 w-full justify-center px-6">
        ...
        return (
            ...
            className={`group flex-shrink-0 
                ...
                max-w-sm bg-white rounded-2xl shadow-xl border border-indigo-600 
                overflow-hidden cursor-pointer transition-transform duration-200 
                hover:scale-105 hover:shadow-2xl relative ${getCardAnimation(idx)} 
                ${isActiveCard ? 'ring-4 ring-[#DD6C73] ring-opacity-30 scale-105 z-10' : ''}`}
            ...
            <div className="relative h-36 w-full overflow-hidden">
                ...
                <div className="absolute top-2 right-2 bg-[#DD6C73] text-white text-xs px-3 py-1 rounded-full shadow font-semibold group-hover:bg-indigo-500 transition">
                Upcoming
                </div>
            </div>
