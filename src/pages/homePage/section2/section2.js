    ...
    return(
        ...
        <div className="section2Container bg-white">
            ...
            <div className="middle">
                <img style={{"background-color": ""}} src={Certification1}></img>
            </div>
          ...
          <div className="statistics absolute md:relative invisible md:visible md:w-full align-items:stretch justify-center flex flex-wrap gap-4">
            {/* Common card classes */}
            {[
              { icon: <FaHandshake className="text-white text-2xl md:text-3xl" />, value: partners, label: "NGO Partners", bg: "bg-[#1A2447]" },
              { icon: <MdVolunteerActivism className="text-white text-2xl md:text-3xl" />, value: members, label: "Members", bg: "bg-[#c4a77d]" },
              { icon: <FcConferenceCall className="text-white text-2xl md:text-3xl" />, value: countries, label: "Countries", bg: "bg-[#1A2447]" },
                  ...
                  <VisibilitySensor partialVisibility offset={{ bottom: 100 }}>
                    {({ isVisible }) => (
                      <p className="text-white font-bold text-xl md:text-2xl">
                        {isVisible ? <CountUp start={0} end={value} duration={2.5} /> : `${value}`}+
                      </p>
                    )}
                  </VisibilitySensor>
                </div>
