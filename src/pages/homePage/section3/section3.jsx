// import "./section3.css";
import EventsList from "../../../components/EventsList/EventsList";
import pastEvents from '../../eventsPage/pastEventsDesc';
import UpcomingEventsSlider from '../../../components/UpcomingEventsSlider/UpcomingEventsSlider';
import { useNavigate } from "react-router-dom";
// Dummy data for upcoming events (replace with real data or import)
const upcomingEvents = [];

function Section3() {
    const navigate = useNavigate();
    return (
        <div className="w-full flex flex-col gap-8">
            <div className="mb-2 flex flex-col flex-1 items-center">
                <h2 className="text-3xl pageTitle font-serif font-bold !mt-0 text-rose-600 mb-2 text-center md:text-left">Past Events</h2>
                <UpcomingEventsSlider events={pastEvents} />
                <button
                    className="!bg-[#DD6C73] hover:!bg-rose-600 cursor-pointer text-sm my-2"
                    onClick={() => navigate('/events')}
                >
                    View All Events
                </button>
                <div className="mt-2 text-center">
                    <p className="text-base text-gray-700 font-medium">To sign up for any events please fill out the 
                        <a 
                        className="inline-flex  gap-1 px-1 text-rose-500 text-navy-300 hover:text-navy-800 hover:underline font-medium" 
                        href="https://docs.google.com/forms/d/e/1FAIpQLSfeEdhXjwm2WP-_6tFYKegiToCikM93qCdg5r4G3dBsWXhcDg/viewform" 
                        target="_blank"
                    >
                        general member form
                        <svg 
                            xmlns="http://www.w3.org/2000/svg" 
                            fill="none" 
                            viewBox="0 0 24 24" 
                            strokeWidth={1.5} 
                            stroke="currentColor" 
                            className="w-5 h-5"
                        >
                            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                        </svg>
                    </a>
                         and all information about upcoming webinars will be forwarded to you!</p>
                </div>
            </div>
            {/* <div>
                <h2 className="text-2xl font-bold text-rose-600 mb-4 text-center md:text-left">Past Webinars & Events</h2>
                <EventsList events={pastEvents}/>
            </div> */}
        </div>
    )
}

export default Section3;